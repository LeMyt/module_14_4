...
async  def send_calories(message, state):
    ...
    data = await state.get_data()
    calorie_intake = 10 * int(data.get('weight')) + 6.25 * int(data.get('growth')) - 5 * int(data.get('age')) - 161
...
async def get_buying_list(message):
    for i in range(4):
        await message.answer(f"Название: {users[i][1]} | Описание: {users[i][2]} | Цена: {users[i][3]}")
